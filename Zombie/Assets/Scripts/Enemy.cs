...
public class Enemy : LivingEntity 
{
    ...
    private IEnumerator UpdatePath() 
    {
        ...
        while (!dead)
        {
            ...
            else
            {
                ...
                // 반경 안 target의 LivingEntity 스크립트 가지고 오기
    ...
    {
        // 1. 자신이 사망하지 않았으며
        // 2. 최근 공격 시점에서 timeBetAttack 이상 시간이 지났다면 공격 가능
        if (!dead && Time.time >= lastAttackTime + timeBetAttack)
        {
            ...
            if (attactTarget != null && attactTarget == targetEntity)
            {
                ...
                Vector3 hitPoint = other.ClosestPoint(transform.position); // 콜라이더 표면 위의 점 중 특정 위치와 가장 가까운 점을 반환.
